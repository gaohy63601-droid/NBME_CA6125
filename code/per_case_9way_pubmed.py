"""9-way per-case ensemble:
   r1 + xl + ms1(greedy 5ep) + ms2(phase2 greedy) + ms3(beam4 5ep)
       + ms4..6 (phase1 sample s42/s100/s200 T=0.7)
       + ms7..9 (phase2 sample s42/s100/s200 T=0.7)

Search space stays tractable by collapsing each sample triple into derived masks:
  msV2_p1  = vote>=2 of phase1 samples       msUNI_p1 = union of phase1 samples
  msV2_p2  = vote>=2 of phase2 samples       msUNI_p2 = union of phase2 samples
  msV2_all = vote>=3 of all 6 samples        msUNI_all= union of all 6 samples
"""
import os, ast
import numpy as np
import pandas as pd
...
xlnpz = np.load(os.path.join(PRED_DIR_R1, "char_probs_pubmed.npz"), allow_pickle=True)
ids = r1npz["ids"]
...
ms_files = {
    ...
    "ms7": "preds_phase2_t07_s42.csv",
    "ms8": "preds_phase2_t07_s100.csv",
    "ms9": "preds_phase2_t07_s200.csv",
}
ms_masks = {}
...
for name in ms_files:
    print(f"  {name} alone F1 = {f1(gms, ms_masks[name]):.4f}")

# Derived sample-aggregate masks
def vec_sum(*names):
    return [sum(ms_masks[k][i].astype(int) for k in names) for i in range(n)]

p1_sum = vec_sum("ms4", "ms5", "ms6")
p2_sum = vec_sum("ms7", "ms8", "ms9")
all_sum = [p1_sum[i] + p2_sum[i] for i in range(n)]

ms_masks["msV2_p1"]  = [v >= 2 for v in p1_sum]
ms_masks["msUNI_p1"] = [v >= 1 for v in p1_sum]
ms_masks["msV2_p2"]  = [v >= 2 for v in p2_sum]
ms_masks["msUNI_p2"] = [v >= 1 for v in p2_sum]
ms_masks["msV3_all"] = [v >= 3 for v in all_sum]
ms_masks["msV4_all"] = [v >= 4 for v in all_sum]
ms_masks["msUNI_all"] = [v >= 1 for v in all_sum]
for name in ("msV2_p1", "msUNI_p1", "msV2_p2", "msUNI_p2", "msV3_all", "msV4_all", "msUNI_all"):
...
# Combo space: keep 5-way's good combos, plus sample-aggregate variants
ms_combo_options = [
    "ms1", "ms2", "ms3",
    ...
    # sample-aggregate
    "msV2_p1", "msUNI_p1", "msV2_p2", "msUNI_p2",
    "msV3_all", "msV4_all", "msUNI_all",
    "ms1|msV2_p1", "ms1|msUNI_p1", "ms1|msV2_p2", "ms1|msUNI_p2",
    "ms1|msV3_all", "ms1|msV4_all", "ms1|msUNI_all",
    "ms3|msV2_p1", "ms3|msUNI_p1", "ms3|msV2_p2", "ms3|msUNI_p2",
    "ms3|msV3_all", "ms3|msV4_all", "ms3|msUNI_all",
    "ms1&msV2_p1", "ms1&msV2_p2", "ms1&msV3_all",
    "ms3&msV2_p1", "ms3&msV2_p2", "ms3&msV3_all",
    "(ms1|ms3)&msV2_p1", "(ms1|ms3)&msV3_all",
    "ms1&msV2_p1|ms3&msV2_p2",
    "vote>=2[1,3,V2_p1]", "vote>=2[1,3,V3_all]", "vote>=2[1,3,V2_p2]",
]
...
def get_extra_mask(combo, i):
    ...
    if "|" in combo and "&" not in combo:
        a, b = combo.split("|"); return ms_masks[a][i] | ms_masks[b][i]
    if "&" in combo and "|" not in combo:
        a, b = combo.split("&"); return ms_masks[a][i] & ms_masks[b][i]
    if combo == "(ms1|ms3)&msV2_p1":
        return (ms_masks["ms1"][i] | ms_masks["ms3"][i]) & ms_masks["msV2_p1"][i]
    if combo == "(ms1|ms3)&msV3_all":
        return (ms_masks["ms1"][i] | ms_masks["ms3"][i]) & ms_masks["msV3_all"][i]
    if combo == "ms1&msV2_p1|ms3&msV2_p2":
        return (ms_masks["ms1"][i] & ms_masks["msV2_p1"][i]) | (ms_masks["ms3"][i] & ms_masks["msV2_p2"][i])
    if combo.startswith("vote>=") and "[1,3,V2_p1]" in combo:
        k = int(combo.split(">=")[1].split("[")[0])
        v = ms_masks["ms1"][i].astype(int) + ms_masks["ms3"][i].astype(int) + ms_masks["msV2_p1"][i].astype(int)
        return v >= k
    if combo.startswith("vote>=") and "[1,3,V3_all]" in combo:
        k = int(combo.split(">=")[1].split("[")[0])
        v = ms_masks["ms1"][i].astype(int) + ms_masks["ms3"][i].astype(int) + ms_masks["msV3_all"][i].astype(int)
        return v >= k
    if combo.startswith("vote>=") and "[1,3,V2_p2]" in combo:
        k = int(combo.split(">=")[1].split("[")[0])
        v = ms_masks["ms1"][i].astype(int) + ms_masks["ms3"][i].astype(int) + ms_masks["msV2_p2"][i].astype(int)
        return v >= k
    raise ValueError(combo)


print("\n--- 9-way per-case search ---")
best_per_case = {}
for c in cases_uni:
    idx = case_idx[c]
...
np.savez(os.path.join(PRED_DIR_MS, "per_case_9way_pubmed_dump.npz"),
...
print(f"\n[9-WAY PER-CASE OVERALL] F1 = {f_overall:.4f}")
print(f"\nbaselines:")
print(f"  5-way per-case (+ beam):                   0.8783")
print(f"  5-way + per-case postproc:                 0.8785")
print(f"  7-way (+3 phase1 samples):                 0.8786")
print(f"  7-way + postproc:                          0.8788")
print(f"  9-way (+3 phase2 samples, this run):       {f_overall:.4f}")
